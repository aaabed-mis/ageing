...
function aggregate(rows){
  ...
  const byBucket={}, byRegion={}, byMgrp={}, byRegionBucket={}, byMgrpBucket={}, byMatnr={}, byMatnrBucket={};
  ...
  return {totalQty,totalVal,expiredQty,expiredVal,batches,deadStockVal,sl01ExpiredVal,coverageDays,byBucket,byRegion,byMgrp,byRegionBucket,byMgrpBucket,byMatnr,byMatnrBucket};
}
...
function bucketTooltip(aggData, bucketKey, tipId){
  ...
  if(!bucketTooltip._globalBound){
    const TIPS=()=>[document.getElementById('region-tip'),document.getElementById('mgrp-tip'),document.getElementById('mat-tip')];
...
function exportCSV(rows){
  const head=['MATNR','MAKTX','WERKS','SALES_ORG','PLANT_CLASS','SLOC','NAME1','REGIO','CHARG','EXT_MAT_GRP','CLABS','UMREZ','NTGEW','WEIGHT_T','MA_PRICE','VALUE','AGING_BUCKET'];
    const cols=['matnr','maktx','werks','bukrs','name2','lgort','name1','regio','charg','extgrp','clabs','umrez','ntgew','weight_t','ma_price','value','aging_bucket'];
...
function refresh(){
  ...
  renderKPIs(a);
  renderBucketChart(a);renderBucketTable(a);renderRegion(a);renderMgrp(a);renderTopMat(rows);
  renderTable(rows);
